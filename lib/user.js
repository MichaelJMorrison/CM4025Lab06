...
export async function updateUser({ username, password, profile }) {
  // This is an in memory store for users, there is no data persistence without a proper DB
  // return users.find((user) => user.username === username);
  ...
  async function run() {
    try {
        ...
        let user_db = await dbo.collection("users").findOneAndUpdate({"username": username}, {$set:{"profile": profile}}, {upsert: true}, function(err, res) {
            if (err) {
                console.log("Didn't find the user")
                console.log(err); 
                throw err;
